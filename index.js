...
console.log("looking for the most recent caption for user: " + senderId);
    ...
    request("https://blooming-wave-81088.herokuapp.com/captions/" + tour + "/" + caption, { description: 1 }, function (error, response) {
            if (error) {
             ...
             var captionRes = (response.body);
               console.log("response is: " + JSON.stringify(captionRes));
               console.log("description is: " + JSON.parse(captionRes).description);
  //       sendMessage(userId,{text: captionRes.body});
                       }
      });
         }
              });
